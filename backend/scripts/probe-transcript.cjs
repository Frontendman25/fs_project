...
const includeAutoGenerated = process.argv[4] !== 'manual'

if (!videoUrl) {
  console.error(
    'Usage: npm run probe:transcript -- <youtube-url> [lang] [manual]'
  )
  process.exit(1)
}

probeTranscript(videoUrl, { lang, includeAutoGenerated })
  .then((result) => {
